...
MergeGuide is SOC 2 Type II certification is planned as a future initiative covering the Security trust service criteria. Contact us at security@mergeguide.ai to request the current readiness status or to discuss audit timelines for procurement purposes.
...
Events include: sign-in/sign-out, member invitation and removal, role changes, SSO/SCIM configuration changes, policy enable/disable, and API key creation and revocation.

Audit logs are retained for 2 years. Export as CSV or JSON for external SIEM ingestion.
