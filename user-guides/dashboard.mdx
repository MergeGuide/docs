...
The bypass rate dashboard shows every instance where a developer pushed code past a failing policy check. This data serves as evidence for SOC 2 CC6.1 (logical access controls monitoring).
...
- **Security** — SSO/SAML, SCIM provisioning, WebAuthn MFA
...
| **Owner** | Admin + delete organization, configure SSO/SCIM |
...
SCIM v2 is available for automatic user provisioning from your IdP. See [SCIM Setup](/enterprise/scim-provisioning).
